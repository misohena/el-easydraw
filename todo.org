...
* Inbox
** TODO svg-imageの使用をedraw-svg-to-imageに置き換える
そうすればsvg.elへの依存は一切無くなるはず。
...
** TODO svg要素のwidth=, height=が単位付きの場合に対応する
そもそもEmacsではどのように扱われるのか。
...
また、プロパティ情報に :to-number という関数があるが、これは廃止したい。おそらくプロパティエディタのホイール・ドラッグ数値変更でしか使っていない。この関数はem、ex、%といった相対指定を処理できない。

** TODO SVGの検証・変換を行う関数を作る
任意DOMツリーがedrawで扱えるかを検証する。

- トップレベル検証
  - svg widthとheightが単位無しまたはpxでなければならない。
  - svg直下にedraw-bodyとedraw-defsがあること。
- 編集対象図形検証
  - 使用される単位は全て無指定であること。
  - スタイル検証
    - スタイルにtransformが使われていないこと。
    - スタイルにwriting-modeが使われていないこと。
  - color (stroke, fill)
  - opacity-value (fill-opacity, stroke-opacity, opacity)
  - stroke-dasharray
  - stroke-dashoffset
  - path d=
  - marker-start
  ...
  - class
  - 使用不可属性
    - clip, clipPathUnits, clip-rule
    - その他基本的に知らない属性は全て

edraw-import-from-svg
...
** TODO edraw-get-actionsを整理する。
[[*選択オブジェクトに対するキー操作をコンテキストメニューに表示する][選択オブジェクトに対するキー操作をコンテキストメニューに表示する]]の続き。

[[elisp-function:edraw-get-actions-for-selected-shapes][edraw-get-actions-for-selected-shapes]]と[[elisp-function:edraw-get-actions][edraw-get-actions]]メソッドを統合したい。可能？

複数選択時のメニュー表示は、edraw-multiple-shapesに対するedraw-get-actionsを使うべきだと思う。

しかし[[elisp-function:edraw-get-actions-for-selected-shapes][edraw-get-actions-for-selected-shapes]]は[[elisp-function:edraw-main-menu][edraw-main-menu]]からも使われているのがネック。
...
** TODO 変形/スタイルシートを考慮する
style属性からでも変形できるので、様々な場面でそれを考慮すべき。
そもそもstyle属性のparserが必要。
...
** TODO edraw-shape-typesの:from-element等は静的メソッドにすべき?
:from-elementや:create-elementに指定する関数はクラスに対する静的メソッドであるべきだろうか。
