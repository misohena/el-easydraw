...
* Inbox
** TODO 細い線のマウス当たり判定を改善する

選択ツールでクリック時の処理の流れ:
1. [[elisp-function:edraw-on-down-mouse-1;method-args=(nil edraw-editor-tool-select t)]]
2. [[elisp-function:edraw-mouse-down-shape;method-args=(nil edraw-editor t)]]
3. [[elisp-function:edraw-find-shapes-by-xy;method-args=(nil edraw-editor t)]]
...
その正方形のサイズは線の太さとピックアップ用半径から決められているが、そこに拡大率が入り込む余地がない。

それどころか、対象のstrokeが未指定の場合はストロークとの当たり判定自体が発生しないようになっている。latexやgridで生成された図形の場合は、ルートのグループにstrokeが指定されているが、その子孫のpath自体には指定されていない。これでは当たるわけがない。
...
- [ ] 修正した関数を使用する部分の動作をテストする。
- [ ] ピックアップサイズを調整する。

** TODO ORG/容量の大きいデータリンクが正しくインライン表示されないことがある
...
おそらく長すぎてfontify範囲の末尾がリンクの末尾より前になっているのだと思う。
...
** TODO 図形生成/プロパティエディタで「-」ボタンが消えることがある
...
** TODO 図形生成/プロパティエディタで何かキー入力で生成したい
C-c C-cで更新できると良いのだけど。
** TODO 図形生成/使用できない生成器を無効化する
無効化できなくても、latexやdvisvgmが入っていない場合は、その旨をユーザーに伝える必要がある。

** TODO グループを境界矩形内のどこでもマウス入力できるようにしたい
細い線が主体の図形の場合選択や移動が大変やりづらい。
編集中の設定で変えられると良い。
もしくは細い線をクリックしやすくするか。

** TODO 任意の要素を一時的に非表示にしたい
** TODO 任意の要素をマウス入力禁止にしたい
編集中に一時的にマウス入力を透過するようにしたい。
もちろんリセットできる。
** TODO 拡大したときに、ストロークの当たり判定が甘い
fillはかなり正確だが、それに比べてstrokeはかなり外れたところでもクリックできてしまう。
アイコンを書くときに困った。
** TODO 矢印キーで1ピクセル未満の移動が出来るようにしたい
設定で矢印キーの移動量を一時的に変更したい
アイコンを書くときに困った。

** TODO キー操作でポイントを平行移動したい
...
** TODO 変形/インタラクティブ変形時にsを押したときyのデフォルトがxにならない
ドキュメントの変形ではxを入れたらyのデフォルトはxになるのに。
