...
* Inbox
** TODO マーカー/アンカーポイントのメニューで設定できるようにする
** TODO edraw-svg-shape-aabbがnilを返す可能性を考慮していないのを修正する

中身が無い図形(空のgroupやpath)の場合nilを返す。
nilの矩形を元に計算をすると算術エラーが発生するので問題。
(~Wrong type argument: number-or-marker-p, nil~ などが発生する)

- edraw-import-from-edraw-svg
- edraw-point-connection-aabb
  - edraw-get-xy => connectionに関する全ての呼び出し元でnilチェックしてる
  - edraw-shape-center
    - edraw-update-src
    - edraw-update
