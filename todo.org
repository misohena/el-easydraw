...
- [ ] edraw-svg-update-marker-properties-in-domはマーカーに限定されないより汎用的なものへ置き換える。
...
** TODO インポート/インポートしたテキストが移動できないことがある
...
** TODO インポート/インポートサブメニューをメインメニューに追加する
ドキュメントの下にすべき？　いや、ドキュメントはドキュメント自体のプロパティに対する操作なのでダメ。
ドキュメントメニューには追加しても良い。
