...
* Inbox
** TODO 変形時にpixel-alignを解除した方が良いのでは？
pixel-alignが適用されている図形は解除してから変形すべきでは？
で、変形後に再適用する？

** TODO ビュー/アンカー等やAABB表示の線がぼやけるのを改善する
これはこれである意味座標を正確に表しているから無理に直す必要は無いかもしれない。

** TODO ビュー/グリッドを最背面に表示するオプションを追加する
うーん、正直私は要らない。
最背面と言っても背景よりは手前にしたいかな？
そう考えるとそんなに良いものじゃないよね。

** TODO ビュー/定規を表示する
** TODO ビュー/ドラッグで表示領域をリサイズしたい
枠を表示して、その右下をドラッグすることでリサイズしたい。
** TODO plistやalistに関する関数を改善する
edraw-alist-やedraw-plist-を使うようにする。

** TODO UIのスタイルシート化
カラーピッカーやエディタ、ツールバー等におけるボタン等について。
色やrx、ry、stroke-width等はスタイルシートで指定すべき。
defconstやdefcustomでの指定も引き続きサポートしたいので、スタイルシート更新関数を作る必要がある。
...
** TODO テキストツールで接着するときのUIを改善する
ミニバッファから接着場所を指定するのは少しつらい。
もう少しGUIでできないものか。
点接続設定UIがあれば良いのだろうか。

** TODO テキスト/テキスト指定で区切り線を引けないだろうか
"--"のような行があったらそこに横線を引くようなことはできないだろうか。
もちろん接着図形があればその辺との交点を結ぶ必要がある。
...
** TODO マーカー/継承が絡むとマーカーの色や太さが更新されない
例えばマーカーがある線をグループ化したとして、その線の色や太さ(stroke-width)は親のグループ要素から継承するとする。親のグループの色や太さを変更した場合、マーカーの更新が行われないので自動的に変化しない。

[[edraw:data=]]
...
** DONE 色/カラーピッカーをキーボードで操作したい
CLOSED: [2025-02-08 Sat 14:41]
- 矢印キーでXY平面(彩度・明るさ)の変更 (Shift+で大きく移動)
  - edraw-color-picker-increment-x-1 (-2)
  - edraw-color-picker-decrement-x-1 (-2)
  - edraw-color-picker-increment-y-1 (-2)
  - edraw-color-picker-decrement-y-1 (-2)
- Ctrl+矢印キーでZ(色相)の変更 (Shift+で大きく移動)
  - edraw-color-picker-increment-z-1 (-2)
  - edraw-color-picker-decrement-z-1 (-2)
- Meta+矢印キーで不透明度の変更 (Shift+で大きく移動)
  - edraw-color-picker-increment-opacity-1 (-2)
  - edraw-color-picker-decrement-opacity-1 (-2)
- 値を入力して設定
  - (C-c C-s) h : edraw-color-picker-set-color-hue
  - (C-c C-s) s : edraw-color-picker-set-color-saturation
  - (C-c C-s) v : edraw-color-picker-set-color-brightness
  - (C-c C-s) r : edraw-color-picker-set-color-red
  - (C-c C-s) g : edraw-color-picker-set-color-green
  - (C-c C-s) b : edraw-color-picker-set-color-blue
  - (C-c C-s) a : edraw-color-picker-set-opacity
  - +(C-c C-s) L : edraw-color-picker-set-relative-luminunce+

変化量はShift+は小さくして、通常は大きくすることも可能。

ミニバッファから打つ場合は C-c C-s を付ける。本当は大文字のHでhueとかでも良いのかもしれない。色指定で大文字を打つことは無いだろうから。
...
** DONE 色/edraw-color-picker-replace-color-atは可能な限り元の記法を尊重する
CLOSED: [2025-02-11 Tue 21:19]
...
edraw-color.elに新しい色テキスト構文処理機能を追加。
...
- Color Syntax
  ...
  - Emacs Syntax
    ...
    - Parse
    - Serialize
  - CSS Syntax ([[https://www.w3.org/TR/css-color-4/][CSS Color Module Level 4]])
    - Color Names
    - Parse
    - Serialize
  - LaTeX Syntax (将来的に)
  - Syntaxタイプ管理 (EmacsやCSSのSyntaxを切り替える仕組み)
  - Syntaxタイプ判定 (バッファローカル変数やメジャーモードから推測)
  - Search (バッファや文字列内にある色テキストを解析する関数)

で、カラーピッカーは置き換えコマンドにおいてこれを使うようにする。

Color Syntax(色構文処理機能)による構文解析は、正規表現の作成(edraw-color-<type>-regexp)とマッチデータの解析(edraw-color-<type>-matched-color-info)の二つの関数によって構成される。これは作成される正規表現はre-search-forwardやlooking-at、string-matchに渡して使う。その正規表現は多少ラフにマッチする場合がある点に注意。それらがマッチしたらマッチデータの解析関数によって色テキストの書き方に関する詳細情報を得る。結果は (color . props) の形。carはedraw-colorオブジェクト。cdrはそれを得るに至るまでに解析された様々な情報が入る。この情報は置き換え先の色テキストを作成するときに利用される。この情報にはマッチテキストの範囲のみならず、使用されている構文の種類やカンマの前後のスペースに至るまで様々な情報が格納されているので、置き換え先のテキストを元のテキストの形式に極力合わせることができる。

残りの課題:
- [X] hslシリアライズ関数の追加。
- [X] 単位の維持 (%が失われる)
- [X] hwb対応。
- [X] その他の色関数の対応 +…は諦めてる。+ はcolor()以外実装した。
- [X] radとかの桁数
- +シリアライズオプションを指定する方法を追加+
- [X] シリアライズオプションを含め色置き換えコマンドのオプションを指定するカスタマイズ変数(edraw-color-picker-replace-color-command-options)を追加する。コマンドとして呼び出したときにinteractive指定を通じて適用される。

- => [[*色/edraw-color-syntax-serializeのoptionsを改善する][色/edraw-color-syntax-serializeのoptionsを改善する]]
- ミニバッファ入力からもこの仕組みを使いたい。現在はミニバッファからはhslが使えない。 => [[*色/新しい色テキスト処理をミニバッファ入力方式にも適用する][色/新しい色テキスト処理をミニバッファ入力方式にも適用する]]
- Custom-mode内からCSSの色をカラーピッカーで指定したい場合もあるのではないか。 => [[*色/color-syntax-systemのフォールバック機構][色/color-syntax-systemのフォールバック機構]]
- => [[*色/新しい構文解析・文字列化機構に合わせて既存の文字列処理を修正][色/新しい構文解析・文字列化機構に合わせて既存の文字列処理を修正]]
...
** DONE when-let/if-letをwhen-let*/if-let*へ置き換える
CLOSED: [2025-02-12 Wed 21:20]
非生産的でバカバカしいけどいずれ必要になるなら何かの合間に直すか…。

- when-let 242 (when-letとwhen-let*を合わせて総数297)
- if-let 66 (if-letとif-let*を合わせて総数80)

私のこれまでの方針では、先に束縛している変数を参照しないなら、つまりlet的な使い方で済むならwhen-letを、そうでなくlet*的な使い方ならwhen-let*を使うようにしていた。when-letがlet的な動作に変わったとしても問題が無いようにするためでもあったし、それでいて*を打たない方がコードが短くスッキリした印象になるからだ。

when-letはwhen-let*とほぼ同じなので、when-letをlet的に並列に複数の変数を束縛するのに使うことはできない(つまり (let ((a 1)) (let ((a 2) (b a)) b)) のようなつもりで (when-let ((a 1)) (when-let ((a 2) (b a)) b)) と書くと痛い目に遭う)。なので、そういう書き方はしていないつもりだし、そうなると単純に ~(when-let (~ を ~(when-let* (~ に置換しても問題ないはず。

ただ、実際にやってみるといくつか問題があった。

- (when-let (a 1111111) a) のように書いてあるケース。
- (when-let ((a 111) (b a)) b) のように書いてあるケース。
- インデントが面倒。

まずは (when-let (a 1111111) a) のように書いてあるケース。aや1111111、BODYのaはもちろんたとえである。1111111の部分は大抵長い式になっていたので、この例でも長くしてみた。docstringを見直すと、when-letはif-letと同じでif-letは(SYMBOL SOMETHING)と書けると書いてある。とは言え私は意図的にこれを書いた記憶は無いので、おそらくミスだろう。幸いコンパイルエラーが出るのですぐに気がつく。

次に (when-let ((a 111) (b a)) b) のように書いてあるケース。私のこれまでの方針では、これはwhen-let*を使うべき所だ。when-letがlet*的な動作をすることは使い始めたときから知っているので、(a 111)の前にあるaを参照する意図ではない。実際大抵は関数内の頭の方にあって、そのような変数は存在しない。これも付け忘れだろう。

インデントが面倒という話については、when-letやif-letのSPECが2行以上にわたっているとwhen-let*やif-let*に置換したときに束縛部分全体をインデントし直す必要がある。まぁ、これは仕方が無い。

こうしてみると、when-letとwhen-let*を使い分けること自体がすでにバカバカしいものであったし、こういったミスも発生していたことも考えると統一したのは良かったのではないか。

置換して一応全部確認した。
** DONE 色/色置き換え・挿入コマンドのマウスイベント処理を修正する
CLOSED: [2025-02-15 Sat 08:11]
他のウィンドウをクリックした時に正しく動かない。
また、コンテキストメニュー経由で実行したときにも正しく動かない。

コンテキストメニュー経由で実行するとlast-input-eventはメニューで選んだメニュー項目のキーイベントになってしまう。一方 ~(interactive "e")~ ではコンテキストメニューを開いたときのイベントが手に入る。これはコマンドを実行したときの一連のイベントの内、最初の1、2個にあるマウスイベントを使っているからだとマニュアルに書いてあった(https://ayatakesi.github.io/emacs/24.5/elisp_html/Interactive-Codes.html)。this-command-keysやthis-command-keys-vectorを使うとその一連のイベントを取得できるので、その中からマウスイベントを探して使うようにした。[[elisp-function:edraw-this-command-event]]を追加した。

最初は -color-at-point内で最後のイベントを参照しようとしたが、それだと (goto-char ..)してから -color-at-point を呼んだときに現在のポイントでは無く最後のイベントの場所に開いてしまう。 -color-at の引数を pos-or-event にして対処した。

** DONE 色/カラーピッカーを簡単に使えるようにするマイナーモードを追加する
CLOSED: [2025-02-15 Sat 08:11]
edraw-color-picker-mode と edraw-color-picker-global-mode 。

edraw-color-picker-modeは単独のバッファ内で動作する。

- defcustomによるキー割り当ての変更機能を提供(特定メジャーモード専用キー割り当て機能あり)
- コンテキストメニュー
...
- 成分を入力
  ...
  - Saturation
  - Brightness
- 増減 (増減のコマンドも一応並べてキー割り当てが分かるようにする)
- 履歴 (transientとminibufferで異なる)
  - 前の色
  - 次の色
** CANCELLED 色/ミニバッファ入力時にキー割り当てヘルプが欲しい
CLOSED: [2025-02-17 Mon 11:13]
今のままだと機能に気がつけないので。

[[*色/カラーピッカーをキーボードで操作したい][色/カラーピッカーをキーボードで操作したい]]
...
** DONE 色/カラーピッカーの書式指定のメニュー項目を増やす
CLOSED: [2025-02-21 Fri 20:44]
- 色名の使用(:color-name-usage): nil, disabled, enabled : FC- FC0 FC1
- 割合の単位(:css-percentage-unit): nil, none, %        : F%- F%0 F%%
- 角度の単位(:css-angle-unit): nil, none, deg, rad, grad, turn : FA- FA0 FAd
** DONE タッチスクリーンからツールバーのボタンが押せないのを直す
CLOSED: [2025-03-01 Sat 20:55]
Emacs 30.1において、ツールバーのボタンをタッチスクリーンから押すとエラーが発生する。
Androidでは押せるがWindowsでは押せない。

次のようなエラーが出る。なぜかエリアが2回出現している。

: <edraw-editor-select-tool-rect> <edraw-editor-select-tool-rect> <mouse-1> is undefined [3 times]
: <edraw-editor-select-tool-select> <edraw-editor-select-tool-select> <mouse-1> is undefined
: <edraw-editor-select-tool-ellipse> <edraw-editor-select-tool-ellipse> <mouse-1> is undefined
: <edraw-editor-select-tool-path> <edraw-editor-select-tool-path> <mouse-1> is undefined
: <edraw-editor-select-tool-freehand> <edraw-editor-select-tool-freehand> <mouse-1> is undefined
: <edraw-editor-select-tool-path> <edraw-editor-select-tool-path> <mouse-1> is undefined
: <edraw-editor-select-tool-rect> <edraw-editor-select-tool-rect> <mouse-1> is undefined

edraw-update-toolbarを修正してツールバーのキーマップに (vector key-id key-id 'mouse-1) を登録してしまえば回避できる。気持ち悪いけど。
** DONE タッチスクリーンに対応する
CLOSED: [2025-03-02 Sun 11:42]
...
- エディタ
  ...
  - インタラクティブ変形
- カラーピッカー
- プロパティエディタのラベルスライダー部分

修正方法:
- edraw-track-draggingをタッチイベントに対応させる
  - track-mouseを使ってもup-mouseイベントが来ない(Windowsだけ)。
- エディタの全ツールのon-touchscreen-beginはon-down-mouse-1を呼び出す。呼び出されたon-down-mouse-1はdown-eventとしてtouchscreen-beginイベントを受け取るが、これまで通りに使用して問題ないはず。
  down-eventに対して行っているのは:
  - event-startを通じて位置を割り出すこと
  - event-modifiersで修飾キーを割り出すこと
  - edraw-track-draggingに渡すこと
- ツールバーが押せない(Windowsだけ) : [[*タッチスクリーンからツールバーのボタンが押せないのを直す][タッチスクリーンからツールバーのボタンが押せないのを直す]]
- edraw-editor-tool-textだけはon-mouse-1を使っているのでon-touchscreen-beginをオーバーライドしてon-mouse-1にたらい回す。

動作が遅い原因の一つは画像の更新間隔。invalidateしたときのタイマーの遅延時間が0なので、画像更新が高頻度で発生してしまうことが避けられていない。タッチイベントはマウスイベントの3～4倍くらいの頻度で発生するが、それでも0秒タイマーの間に隠れることは出来ないので。まだpathツールやread-rectangleが遅いので、おそらく他にも原因があると思う。
** DONE 画像更新の間隔を延ばす
CLOSED: [2025-03-02 Sun 14:08]
現在はタイマーで0秒で更新しているが、これを適切な長さに伸ばす。
タッチイベントはマウス移動イベントの3～4倍ほど多く発生するため、非常の高頻度が画面更新が発生して全体の動作が非常に遅くなってしまう。
タイマーの遅延時間を0.008にするだけでも状況が改善する。
カラーピッカーにも同じ問題がある。

課題: [[*画像更新間隔の管理部分を改善する。][画像更新間隔の管理部分を改善する。]]
** DONE 画像更新間隔の管理部分を改善する。
CLOSED: [2025-03-02 Sun 20:49]
- 前回の更新時間を元に遅延時間を決定する方が良い。
- 更新間隔はカスタマイズ出来る方が望ましい。(どこにカスタマイズ変数を作るか迷う。edraw-util?)
- edraw-editorとedraw-color-picker-display-overlayでコードが重複しているのでまとめたい。(edraw-utilに?)

edraw-util.elにedraw-lazy-image-updatorクラスを追加した(edraw-util.elにeieioを入れたくなかったけど仕方ない)。edraw-editorとedraw-color-picker-display-overlayはこれを継承するようにした(ある意味実装継承っぽいが、必ずしもそうとは言い切れない気もする。edraw-invalidate-imageやedraw-update-image-timer-cancelがオブジェクトに対して使えるのは気持ちよい)。

カスタマイズ変数を追加:
- edraw-image-update-interval
- edraw-image-update-fixed-delay-p

デフォルトは平均的な更新間隔が1/60になるようにした。
** DONE タッチのスワイプ(ドラッグ)操作時の遅延によるフリーズを何とかする
CLOSED: [2025-03-03 Mon 14:55]
カラーピッカーで顕著。手書きツールもつらい。

touchscreen-updateはmouse-movementに比べると大幅に発生頻度が高く、スワイプ時に処理が間に合わないと未処理のイベントが溜まり、それが全て処理し終わるまで何秒でも操作できなくなってしまう。

大幅な遅れを検出して移動イベントを捨てるような処理を入れたい。

また、カラーピッカーやread-rectangle等では移動経路を正確に記録する必要が無い(手書きツールは別)。なので、連続する複数の移動イベントは最後の移動イベントだけを処理すれば十分。その方が効率が良くなる。

遅れを検出する方法:
- ×(read-event nil nil 0) => 溜まっているはずのタッチイベントを一度に取得することは出来なかった(少なくともWindowsでは)。
- △posn-timestamp(システム依存のミリ秒)と現在時刻との差を監視する => 可能ではあるが全てのプラットフォームでうまく行くかは分からない。
- ×run-at-timeで少し時間をおいてからまとめて処理する => 5ms程度の遅延では1つずつしか処理できなかった。10～100msくらいであればある程度まとめて処理できるが、遅延が大きくなってしまうのであまり良くない。
- ○run-with-idle-timerでアイドルになったらまとめて処理する => 一番安定して処理できた。
** DONE edraw-track-dragging関数を整理する。
CLOSED: [2025-03-03 Mon 15:57]
- edraw-track-dragに改名する。
- キーワード引数にする。
- keep-echo-areaはnot-keep-echo-areaに改名する。デフォルトで維持する。イベントのエコーが必要な場所は無いと思う。
- allow-skipping-delayed-movesはneed-intermediate-pointsに改名する。デフォルトで不要。ほとんどの場所で途中のポイントは必要ない。必要なのはフリーハンドツールだけ。
- mouse-fine-grained-trackingは常にtにする。現在はプロパティエディタとカラーピッカーで使っている。
** DONE edraw-y-or-n-pとy-or-n-pが混在している
CLOSED: [2025-03-03 Mon 16:47]
Androidではedraw-y-or-n-pが正しく機能しない。プロンプトが全て表示されないのと、はいといいえの日本語が文字化けしている。普通にy-or-n-pを使うべきではないか？

- 実装をedraw-y-or-n-p--popup-menuに移動する
- edraw-y-or-n-p-function変数を追加して切り替えられるようにする(Windows以外ではデフォルトはy-or-n-pにする)
- edraw-y-or-n-p--popup-menuも(edraw-use-dialog-box-p)がnilのときはy-or-n-pを使う
- 基本的にedraw-y-or-n-pを使う
- ただし、マウスを使うことはほぼ無い場所ではy-or-n-pを使う
** DONE shapeのコンテキストメニューからの削除は確認せずに削除する
CLOSED: [2025-03-03 Mon 17:20]
現状でも複数選択してからだと確認しない。おそらくmultiple-shapesのではなく、選択図形の削除メソッドが呼び出されているから。同様にキーボードからだと確認しない。
Undoがあるから不要だと思う。
Delete...ではなくDeleteにする。
edraw-removeを使う。
edraw-delete-with-confirmはどこからも使われなくなる。
** DONE Androidでショートカットキーが効かない
CLOSED: [2025-03-04 Tue 14:39]
pp(プロパティ一覧)やz(UNDO)等が効かない。矢印キーなどは効く。

https://sourceforge.net/projects/android-ports-for-gnu-emacs/files/ のREADMEの「10.My package X that relies on reading raw key events / binding keys globally doesn't work with an on screen keyboard!」は関係ある？ ([[https://qiita.com/ayatakesi/items/87828596c7fa136ac680#10-%E7%94%9F%E3%81%AE%E3%82%AD%E3%83%BC%E3%82%A4%E3%83%99%E3%83%B3%E3%83%88%E3%82%AD%E3%83%BC%E3%83%90%E3%82%A4%E3%83%B3%E3%83%89%E3%81%AB%E4%BE%9D%E5%AD%98%E3%81%99%E3%82%8B%E3%82%8F%E3%81%9F%E3%81%97%E3%81%AE%E3%83%91%E3%83%83%E3%82%B1%E3%83%BC%E3%82%B8x%E3%81%AF%E3%82%AA%E3%83%B3%E3%82%B9%E3%82%AF%E3%83%AA%E3%83%BC%E3%83%B3%E3%82%AD%E3%83%BC%E3%83%9C%E3%83%BC%E3%83%89%E3%81%A7%E3%81%AF%E6%A6%82%E3%81%AD%E5%8B%95%E4%BD%9C%E3%81%97%E3%81%AA%E3%81%84%E3%81%AE%E3%81%A0%E3%81%8C][和訳]])

text-conversion-styleをnil(set-text-conversion-style nil)にすると効くようになる。

text-conversion-styleはバッファ内でのIMEによる変換をコントロールするためのものらしい。nilなら無効、tなら有効、actionはshellなどのコマンドラインで使われていてRETの扱いが変わるらしい。

text-conversion-styleは各major-modeで必要なら設定するものらしい。例えばprog-modeでtにしているので、emacs-lisp-mode等各種プログラミングモードではtになる。text-modeでもtにしているので、outline-modeやorg-modeでもtになる。diredやedraw-modeでは何も設定していないのでnilだからショートカットキーは効く。

org-speed-commandも同様に効かなくなっている。スピードコマンドはself-insert-commandをorg-self-insert-commandに置き換えることで実現している。text-conversion-styleがtの時はこれは呼ばれずに文字が挿入されるので効かなくなる。

edebugも操作不能になる。

結局の所、org-mode用に別のキーバインドを用意した方が良いかもしれない。つまり専用のプレフィックスを決める。使えるプレフィックスというのも多くは無いのだけど、例えば C-. あたり。このキーは私はEmbarkで使っているが、edrawの編集中にEmbarkを使いたいということはあまりないのではないか？(あり得ない話じゃないが)

C-.という短いプレフィックスとは言え、zだけでUNDO出来ていたのに比べるとやはり面倒だ。なんとか編集中だけ効くようにならないだろうか。無理矢理やるならeditorの上にいる間だけ(set-text-conversion-style nil)にするくらい。post-command-hookで開いているエディタ上にポイントがあるならnilにする。そのようなマイナーモードを作るくらいしか思いつかない。他に見落としている仕組みが無ければ。

というわけで edraw-org-text-conversion-control-mode を追加した。
** DONE 色/子フレームにフォーカスがあるときの問題を修正する
CLOSED: [2025-03-04 Tue 19:43]
子フレームが選択されているとC-gで脱出できない。
そもそも子フレームが選択されるべきでは無いので、できるだけ子フレームからフォーカスを親フレームに移す。
親フレームにあるときでもC-gで挿入・置き換えた色がUndoされるようにする。
** DONE 色/header-line-formatが使われていると子フレームの位置がずれる
CLOSED: [2025-03-04 Tue 19:52]
(window-inside-pixel-edges)はヘッダーやフリンジ等を含まない純粋なbodyの範囲を返すのに対して、(pos-visible-in-window-p (point) nil t)はフリンジを除くがヘッダーは含む座標、つまりx=0がフリンジの右で、y=0がヘッダーの上であるようなよく分からない座標系で返してくることが原因。

上下方向がおかしいのは致命的で、場合によってはポイントや色テキストがカラーピッカーによって隠れてしまう。

仕方ないので、(window-pixel-edges)を使うようにした。これだとフリンジや右にあるスクロールバーに重なってしまうが、まぁ、別にいいか。
** DONE ポイントがedraw-editor上に無いときにタッチで絵が描けない
CLOSED: [2025-03-04 Tue 21:08]
edraw-editor-at-inputがタッチイベントを認識していないのが原因。現在のpointによる検出にフォールバックして失敗している。
** DONE Emacsの配色に合わせてUIの色をカスタマイズ出来るようにする
CLOSED: [2025-05-23 Fri 11:41]
[[https://github.com/misohena/el-easydraw/issues/39][feature request: customizable colors in edraw-color-picker · Issue #39 · misohena/el-easydraw]]

色がハードコードされている部分が多いので。

方法はいくつか考えられる。
- classに基づくカスタマイズ
  - 各UIパーツにclassを指定してスタイルシートでカスタマイズする。
  - SVG要素作成関数に確実にclassを指定して、その関数にadviceをかければ済むようにする。
  - UIデフォルト属性alistを用意する。
- 単に色指定文字列をdefconstに置き換える。

rxやry、stroke-width等もハードコードされているが、これもカスタマイズ出来るようにするならclassベースの方法が良いだろう。

とは言えclassで解決できるのは結局CSSプロパティで出来るレベルまでなのでどこまでもカスタマイズ出来るわけではない。

単に色を調整したいだけならdefconstで十分だろう。
** DONE 色/edraw-color-picker-modeのコンテキストメニューを改善する
CLOSED: [2025-12-08 Mon 17:18]
メニューの下の方に項目を追加する。
add-hookのdepthを大きくしただけでは解決しない。
define-key-afterを使えば良い。
** DONE 色/カラーピッカーの現在値表示とヘルプを改善する
CLOSED: [2025-12-08 Mon 21:52]
- 色の置き換えを行うときに、最初から現在値が知りたいことがある。.で表示する。
- Labの輝度を知りたい。
- 現在値を表示中に?でヘルプを出す。
** DONE 色/lab形式等で要素間に空白が入らないことがある
CLOSED: [2025-12-08 Mon 23:48]
例えば rgb(255,0,0) のような色をカラーピッカーで置き換えるときに出力形式としてLABを選ぶと lab(53.2480.167.2) になってしまう。元の形式に空白が無いので空白が入らないのだと思う。

原因はpre-spacesとpost-spacesの役割分担が曖昧だったこと。引数間には、前の引数のpost-spaces、次の引数のpre-separator、次の引数のpre-spaces、の要素が存在している。legacy形式はseparator(,)が存在しているのでpre-spacesとpost-spacesの両方が無くても問題ないが、modern形式はどちらも無いと空白が出力されず区切りが無くなってしまう。

legacy形式のpre-spacesとpost-spacesはmodern形式を出力する際にどう取り扱えば良いのだろうか。二つを両方出力し、一つもスペースを出力しなかったら最低限の空白を追加するというのが一つのアイデア。しかし本来カンマがあることを前提にしたpre-spacesとpost-spacesをそのまま出力するのはおかしいかもしれない。modern形式を解析した場合、引数間にはpre-spacesは発生しない。だからpre-spacesは必ず無視して捨ててしまうというのはどうだろう。post-spacesだけ使用して、それが空なら最低限の空白を挿入するというのはどうだろう。post-spacesすら無視して単に空白を一つ追加するというのも手。

うーん、とりあえずpost-spacesだけ使うようにする。
** DONE 色/カラーピッカーをドラッグで移動できるようにする
CLOSED: [2025-12-09 Tue 11:02]
見たい物(エコーエリア(現在値やヘルプ)やバッファ内の他の情報など)に重なって見えなくなることがあるので。
header-lineを有効にするのが一番簡単だが、ボタンがないところでドラッグさせたい。el-vkbdで似たようなことは既にやっている。
** DONE 色/edraw-color-picker-mode-mapの無効なメニュー定義を削除する
CLOSED: [2025-12-09 Tue 15:16]
edraw-color-picker-mode-mapになぜか無効なメニュー登録がある。
...
これのせいで、コンテキストメニューにマイナーモードメニューを出そうとして出せなくてセパレータだけが無駄に挿入されてしまう。

次のようにすれば一応メニューバーにメニューは追加されるが、メニューバーにわざわざエラーピッカーメニューを追加したいとは思わない。
...
(defvar edraw-color-picker-mode-map
  (let ((km (make-composed-keymap
    ...
    (define-key km
                [menu-bar edraw-color-picker-mode]
                ;;↓ここを修正
                (cons "Color-Picker" edraw-color-picker-mode-menu-map))
    km)
  "The keymap used by `edraw-color-picker-mode'.
\\{edraw-color-picker-mode-map}")
#+end_src

現在カラーピッカーメニューはToolsメニューの下に追加しているが、何か勘違いがあったのだろうか。それとも消し忘れだろうか。今となっては分からない。

シンプルにdefine-key部分だけを削除しておく。
** DONE Version 1.3.0
CLOSED: [2025-12-13 Sat 21:21]
- edraw.elのPackage-Versionを1.3.0にしてコミットする。
- タグを打つ。 git tag -a v1.3.0 -m "Version 1.3.0"
- edraw.elのPackage-Versionを1.3.1にしてコミットする。
** DONE 細い線を描くために0.5pxずらす
CLOSED: [2025-12-14 Sun 18:40]
1pxの直線がぼけるのは昔から分かっている問題のはずだけどなんで直さなかったんだろう。

あー、そうか、単純に全体を0.5pxずらすと線の幅を2pxにしたら今度は3pxに跨がってしまうからか。要するに奇数ピクセル幅のときだけ調整しないといけないんだった。

draw.ioはそれをやってる。線幅が奇数の時だけ ~transform="translate(0.5, 0.5)"~ という属性が入る。いや、厳密には四捨五入して奇数の時、かな。いやいや、ビューの拡大率によっても変わるぞ。まぁ、そこまではする必要は無いだろう。拡大したら線がぼやけるのは自然なことだし。

Illustratorには「Align to Pixel Grid」という機能があるみたい。

理想的には内部的に丸めていない元の座標を持っておいて、それを整数に丸めて使うというようなことが考えられる。でもSVGデータレベルでそれをやるのは厳しい。ストロークも単純なfillだけのパスに分解した後であれば、全ての座標を丸めれば済むけれど。ストロークをそのままの形で取り扱いつつ、そのアウトラインをピクセルグリッドに合わせるのは厳密にやるのは難しい。

線幅によってtransformを自動的に付加する機能をedraw-dom-svg.elに付けるのが良いと思う。

- pixel-align-enabledというプロパティを追加する。data-edraw-pixel-align-enabled="default|true|false"
- pixel-align-enabledがtrueまたは、defaultかつエディタの設定が有効ならの時はtransformを調整し、noneの時は調整しない。
- 調整はtransform属性の先頭にtransform(0.5, 0.5)を付けることによって行う。data-edraw-pixel-aligned="true"も付ける。
- transformプロパティは常に調整前の値を返し、受け付ける。
- stroke-widthプロパティが変化したらtransform属性も更新する。

理想的には何の属性も付けたくないのだけど可能だろうか。つまり偶数幅→奇数幅のときに+0.5ずらし、奇数幅→偶数幅のときに-0.5ずらす。これはスナップ機能に近い。線の幅を変えるときに線のアウトラインがピクセルグリッドに合うように調整を加えるということ。そしてその結果が正式な状態になる。全てはエディタ側の責任。座標の小数は考慮しなくて良いと思う。小数を入れたらぼけるのは自然だし、むしろぼけないと段階的にずらした線を並べたときに連続的に変化してくれない。なのでこの機能はあくまで「線幅による」ピクセルグリッド合わせ。

transformによるスケーリングや回転、せん断変形の影響はどうだろう。真面目に縦横の拡大率を計算してずらす量に反映してもいいけど限界はある。

いっそのことtransform属性が指定されていないときだけ調整を行うというのはどうだろう。つまり奇数幅でtransform属性が無いならtranslate(0.5,0.5)を付け加え、偶数幅でtranslate(0.5,0.5)があるなら取り除く。それだけ。シンプル。transformが無い状態や0.5ずらす指定がユーザーの意図だった場合はそれを破壊することになるけれど、自動的にピクセルアラインを調整するというのはそういうことだから別によいのかもしれない。回避策は色々ある。transform(0,0)を指定すれば自動調整を抑制できる。0.5ずらすにしてもmatrix(1 0 0 1 0.5 0.5)と書けば勝手に取り除かれたりしない。

edraw-svg-element-set-propertyがキモかな。呼び出しは作成時と変更時のみ。呼び出し側でやっても良いが、この中でやっても良い。どうしよう。呼び出す前にprop-listにtransformを追加してしまう。

その追加処理をするかどうかの設定をeditorのsettingsに持たせる。そのものズバリpixel-align。そしてメニューから変更できるようにする。「デフォルト設定」のところでいいかな。設定はui-stateに保存する。この辺りは後で必ず保存するか設定できるようにした方が良いかもしれない。
** DONE グリッド線を0.5pxずらす
CLOSED: [2025-12-14 Sun 19:26]
前からぼやっとしていて気持ち悪いと思っていたんだよね。
shapeの方を補正する機能を追加したので、それに合わせてこちらも直す。

線の太さはCSSで指定するから分からない。
単純にずらすかどうかを指定するカスタマイズ変数を追加する。
edraw-editor-grid-pixel-alignを追加。

** DONE 太い(濃い)グリッド線を描画する
CLOSED: [2025-12-14 Sun 22:05]
濃い線を主グリッド線(major grid lines)/薄い線を副グリッド線(minor grid lines)と呼ぶことにする。
Adobe製品始め多くの製品は主グリッド線の間隔を設定し、その中を何分割するかを指定しているみたい。
しかし私は誤差が怖いし面倒なので、グリッド線何本毎に主グリッド線を入れるかを指定させることにする。
ついでに線の色や濃さも調整する。間隔は10pxにしようとおもったけど重くなるので20pxのままにした。

edraw-editor-default-grid-major-periodを追加。

** DONE ピクセルグリッド合わせの問題を何とかする。
CLOSED: [2025-12-15 Mon 21:15]
[[*細い線を描くために0.5pxずらす][細い線を描くために0.5pxずらす]]で生じた問題。
図形やアンカーポイントを動かすとせっかくずらしたのがグリッドに吸い寄せられ無駄になってしまう。
また、図形の移動はtransform(0.5,0.5)があるせいでtransformを使った移動になってしまうようだ。ドラッグでも矢印キーでもそうなる。

おそらくあちこちに条件を満たしていたら0.5ピクセルずらす処理を入れれば良いのだと思う。

とりあえずドラッグするところを全部見てみよう。

- ハンドルやアンカーの移動
  - [[elisp-function:edraw-mouse-down-handle-point;method-args=(nil edraw-editor t)]]
  - [[elisp-function:edraw-mouse-down-anchor-point;method-args=(nil edraw-editor t)]]
  - [[elisp-function:edraw-drag-handle-on-click-anchor]]
  - [[elisp-function:edraw-create-symmetrical-handles-on-anchor]]
- shapeの移動(対処不要)
  - [[elisp-function:edraw-mouse-down-shape;method-args=(nil edraw-editor t)]]
  - [[elisp-function:edraw-mouse-drag-shape-duplicate;method-args=(nil edraw-editor t)]]
- edraw-shape-transformer系(対処不要)
  - [[elisp-function:edraw-scale-by-dragging;method-args=(nil edraw-shape-transformer t)]]
  - [[elisp-function:edraw-set-transform-origin-by-dragging;method-args=(nil edraw-shape-transformer t)]]
  - [[elisp-function:edraw-translate-by-dragging;method-args=(nil edraw-shape-transformer t)]]
  - [[elisp-function:edraw-rotate-by-dragging;method-args=(nil edraw-shape-transformer t)]]

shape全体を平行移動する部分は対処不要だが、edraw-transform-autoを改良する必要がある。平行移動のみの行列が設定されている場合は、アンカーポイントを平行移動すべき。

後はドラッグ以外にも問題があるかどうか。見当たらないのでとりあえずここまで。
** DONE グリッド間隔をエディタセッションを跨いで記憶したい
CLOSED: [2025-12-16 Tue 00:14]
...
[2025-12-16 Tue 00:14]ちゃんと保存するようにした。
** DONE ユーザー設定を保存・復元すべき
CLOSED: [2025-12-16 Tue 00:16]
グリッド線の表示や間隔等を保存・復元すべき。

関連TODO:
- [[*グリッド間隔をエディタセッションを跨いで記憶したい][グリッド間隔をエディタセッションを跨いで記憶したい]]

pixel-alignはすでにそうなっている。
ただ、こういった設定を保存するかどうかの設定も欲しくなるかもしれない。
そのあたりはどうすれば良いのだろう。
ui-stateはプロパティエディタやシェイプピッカーなんかでも使っているはず。
何か一貫性のある仕組みが必要かもしれない。

カスタマイズ変数[[elisp-variable:edraw-editor-setting-persistence-alist]]を追加した。
** DONE 点接続/接着した1pxの線がぼやけるのを改善する。
CLOSED: [2025-12-16 Tue 11:35]
1pxの矩形同士を1pxの線で繋げると1pxの線がぼやけてしまう。対象の矩形が0.5pxずれているので、それを繋ぐ線も当然0.5pxずれる。その上で0.5pxずらすから。pixel-align設定が有効ならこれも整数座標に丸めた方が良いかもしれない。

単純に他に接着している線にに対してpixel-alignを適用するのは良くない。接着対象がすでに(0.5,0.5)ずれている。それに合わせて位置を計算しているのに、translate(0.5,0.5)を適用した上にさらに(0.5,0.5)ずらすと合計(1.5,1.5)もずれてしまう。

むしろ(-0.5,-0.5)ずらすべきだが、何かがおかしい。

案:
- 接着対象の本来位置に対して接着位置を計算する。
- 点接続を持つ図形はpixel-alignしない。
- 事後的に(-0.5,-0.5)ずらす。
- 接着位置を整数座標に丸める。

接着対象の本来位置に対して接着位置を計算すると、接着対象のstroke-widthが1pxで線が2pxの時に(-0.5,-0.5)ずれた位置に接着してしまう。

点接続を持つ図形をpixel-alignしないのは一見楽そうだが、点接続の状態を変更したらpixel-alignによるtransformを更新しなければならなくなる。

pixel-alignは整数座標に置くことが前提の処理。だから線がpixel-alignされているなら、座標を整数に丸めてやるのが良い。ただ、結局理想的な接着位置から(0.5,0.5)ずれているので、その上で丸めると最大(1.0,1.0)のずれが発生する。

pixel-alignが適用されている時点で理想的な接着位置よりも(0.5,0.5)ずれているのだから、その分を差し引いてから整数に丸めるのが良いのでは無いか。そうすれば理想的な位置からのずれは最小限でぼけもある程度防止できる。1px未満の細かい作図はうまくいかなくなるが、どのみちその場合はpixel-alignを無効にすべきだろう。
** DONE パスを複数選択して終端マーカー→その他を選んでも何も表示されない。
CLOSED: [2025-12-16 Tue 14:25]

必要なコマンドが定義されていなかった。メニューのコマンド名もおかしい。

次のメソッド・コマンドを追加してメニューに登録した。
- edraw-(editor-)set-marker-start-by-menu-selected
- edraw-(editor-)set-marker-end-by-menu-selected
** DONE マーカー/サイズを変更したときに線の太さが変わらないようにしたい
CLOSED: [2025-12-17 Wed 13:36]
1px幅の線を引いたときに2px幅のときでも小さかったマーカーがよりいっそう小さくなってしまう。
修正しようとmarkerWidthとmarkerHeightを調整するとマーカーの線の幅が太くなってしまう。
マーカーの線の幅を維持したまま大きくしたい。
マーカーのstroke-widthは別途調整できても良い(現在はstroke-width="1"固定)。
つまりマーカープロパティmarkerWidth・markerHeightを変えたときに図形形状の倍率だけ変わって線の太さが変わらないようにしたい。

+本来は線の幅を変えてもマーカーの大きさも変わらないようにした方が良いのでは？　まぁ、今変えると互換性上問題か？　markerWidthとmarkerHeightを変えたときの太さも互換性上問題があると言えばあるけど、太さが変わってしまうのを我慢して使っていた人も多いだろうから別にいいんじゃないかな。+ この問題にも対応した。

修正すべき所:
- edraw-svg-marker-path-create
- edraw-svg-marker-path-overhang
- edraw-svg-marker-types

実装:
- markerWidthとmarkerHeightに合わせてパスデータそのものをスケーリングする。同時にviewboxもそれに合わせてスケーリングする。
- overhang量の計算はmarkerWidth・markerHeightによって大きくなる分とstroke-widthによって大きくなる分を分ける必要がある。

[[edraw:data=]]

例えばhollow-circleの場合、先端の位置は 4*水平図形拡大率 + 0.5*(stroke-width) となる。水平図形拡大率は、markerWidth / 100 にstroke-widthを乗じた値になる。ただし、markerUnitsに"userSpaceOnUse"を指定する場合はstroke-widthを乗じる必要は無い(stroke-widthによって勝手に大きくならないので)。

[[edraw:data=]]

図形によってstroke-widthが先端位置に与える影響は異なるので注意すること。

[[edraw:data=]]

ついでに edraw-svg-marker-size-independent-of-stroke-width を追加して、マーカーサイズがstroke-widthの変化によって変わらないように設定できるようにした。デフォルトはt。1pxの線を2pxに変えたらマーカーが2倍の大きさになるなんてちょっと馬鹿げてる。

線幅が2pxの時に2倍にならなくなったので、現状のマーカーではサイズが小さすぎる。
デフォルトで今の2倍の大きさに変更する。
ついでにarrowやcircleもそれ以降に追加したマーカーと同じ方式に切り替える。
これによってmarkerWidthとmarkerHeightプロパティの値に互換性がなくなった。
問題があったらマーカーを作り直すように。
** DONE 線幅1pxの矩形に接着した2pxの直線がぼやけるのを改善する
CLOSED: [2025-12-17 Wed 15:23]
↓これ

[[edraw:data=]]

原因に心当たりはある。今のうちなら。あともう1～2ヶ月もすれば忘れるね。

点接続のpixel align処理は edraw-adjust-point-connection-xy-for-pixel-align で行われている。ここでは接続元図形がpixel align適用済みなら移動先座標を調整し、それ以外の時は調整していない。editorがpixel alignを要求するなら、最低限整数座標に丸めるべきだろう。

[[edraw:data=]]
** DONE 手書きツール/速度の改善をする
CLOSED: [2025-12-22 Mon 17:38]
一筆で長い線を描いているとだんだん遅くなってくる。飛び飛びの線になっていく。
プロファイルしたところedraw-add-anchor→edraw-update-path-d-attr→edraw-path-data-to-string→edraw-path-data-to-string→edraw-path-data-stringize-concat-cmdのあたりが遅い。

1点追加する毎にd=属性をまるごと再生成して最適化するのだから遅いに決まっている。単純に座標だけを文字列の末尾に追加すべきだ。それでもだんだん遅くなる(長い文字列の末尾に何かを追加するのは遅いはず)なら手頃なところでpath要素自体を分ければ良い(おそらくそこまではする必要は無い。どのみちSVG全体の文字列結合は避けられないし、それが発生してもそこまでパフォーマンスは悪化していないのだから)。

** DONE org-mode 9.8に対応する。
CLOSED: [2026-02-22 Sun 13:53]
edraw-org-edit.elで警告が出る:

: In edraw-org-edit-regular-file-link:
: edraw-org-edit.el:231:24: Warning: ‘org-display-inline-images’ is an
:     obsolete function (as of 9.8); use ‘org-link-preview-region’ instead.
:
: In edraw-org--remove-org-inline-images:
: edraw-org-edit.el:244:22: Warning: ‘org-inline-image-overlays’ is an
:     obsolete variable (as of 9.8); use ‘org-link-preview-overlays’ instead.
: edraw-org-edit.el:245:50: Warning: ‘org-inline-image-overlays’ is an
:     obsolete variable (as of 9.8); use ‘org-link-preview-overlays’ instead.
: edraw-org-edit.el:245:15: Warning: ‘org-inline-image-overlays’ is an
:     obsolete variable (as of 9.8); use ‘org-link-preview-overlays’ instead.

インライン画像機能がより一般化されたリンクプレビュー機能に置き換わったことが原因。

基本的に edraw-org-edit-regular-file-link にしか影響しない。この関数はfileリンクでedrawを使うときのための関数。私は普段使っていない。

一応直しておく。

** DONE Emacs31での警告に対処する。
CLOSED: [2026-08-19 Wed 20:02]

- dom-text => dom-inner-text  (textオブジェクトのSummaryにしか使っていない。dom-inner-textはEmacs31からしか使えないので注意)
- when-let => when-let* (1箇所残っていた)
