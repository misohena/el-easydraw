...
変換は次の点に留意する:
- 必要な要素(rect、ellipse、path、text、image、g)以外は全て破棄する。descやtitle等は要らない。
- defsは一応残す。(SVG直下にまとめて配置する？　専用のgの中に入れる？)
- 必要な属性以外は全て破棄する。
- polyline、polygonはpathへ変換する。
- circleはellipseへ変換する。
- 複数のサブパスを持つパスデータは警告を出す。
- style属性を使っていたら警告を出す。
- textはテキストノードが入っているのみの単純なもの以外は色々警告を出す。
- 
