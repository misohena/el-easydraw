...
** TODO タッチ更新やマウス移動イベントの実行を遅延したい。
カラーピッカーやread-rectangle等では移動経路を正確に記録する必要が無い(freehandツールは別)。なので、連続する複数の移動イベントを一つにまとめた方が効率が良くなる。タッチパネルで移動イベントの処理が追いつかないと処理の遅れがかなり目立つので。
