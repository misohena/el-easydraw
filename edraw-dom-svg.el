...
(defun edraw-dom-first-node-p (dom element)
  (if-let ((parent (dom-parent dom element)))
      (eq (car (dom-children parent)) element)
    t))

(defun edraw-dom-last-node-p (dom element)
  (if-let ((parent (dom-parent dom element)))
      (eq (car (last (dom-children parent))) element)
    t))

(defun edraw-dom-reorder-prev (dom element)
  (when-let ((parent (dom-parent dom element)))
    (let ((index (seq-position (dom-children parent) element #'eq)))
      (when (> index 0)
        (let* ((prev-cell (nthcdr (1- index) (dom-children parent)))
               (prev-node (car prev-cell)))
          ;; depends on dom.el node structure
          (setcar prev-cell element)
          (setcar (cdr prev-cell) prev-node))
        t))))

(defun edraw-dom-reorder-next (dom element)
  (when-let ((parent (dom-parent dom element)))
    (let* ((index (seq-position (dom-children parent) element #'eq))
      ...
      (when next-cell
        ;; depends on dom.el node structure
        (setcar next-cell element)
        (setcar curr-cell next-node)
        t))))

(defun edraw-dom-reorder-first (dom element)
  (when-let ((parent (dom-parent dom element)))
    (when (not (eq (car (dom-children parent)) element))
      (dom-remove-node parent element)
      (dom-add-child-before parent element (car (dom-children parent)))
      t)))

(defun edraw-dom-reorder-last (dom element)
  (when-let ((parent (dom-parent dom element)))
    (when (not (eq (car (last (dom-children parent))) element))
      (dom-remove-node parent element)
